...
Log in to your Supabase account at app.supabase.com.
Click New Project, select your organization, and give your project a name (e.g., ssw-bookhub).
Choose a region close to you and set a secure password for the database.
Wait for the project to be created (this may take a minute).


Get Supabase API Keys:

Go to your project dashboard in Supabase.
Navigate to Settings > API.
Copy the URL (e.g., https://your-project-id.supabase.co) and the anon key (a long string under API keys).


Configure supabaseClient.js:

In the project directory, locate the file supabase/supabaseClient.js (or create it if it doesn’t exist).

Open the file in your code editor and update it with your Supabase URL and anon key. It should look like this:
...
You’ll need to create tables for users, products, orders, order_items, comments, and order_messages. Below is the SQL schema to set up these tables. Copy and paste this into the SQL Editor and click Run.
...
After running the SQL script, go to Table Editor in Supabase.

You should see the tables (users, products, orders, etc.) listed.

Optionally, insert some test data (e.g., a few products) to use while testing the app:
...
The app will load in Expo Go. If it doesn’t load, ensure your device and computer are on the same Wi-Fi network.
...
In the Expo terminal, press a to run the app on the Android emulator.


For iOS (macOS only):
Open Xcode and start an iOS simulator.
In the Expo terminal, press i to run the app on the iOS simulator.
...
Update supabaseClient.js to use these variables:
import { createClient } from '@supabase/supabase-js';
